...
//
// A deadline is not a cancellation: once it passes, another worker may take the task over, and the
// original executor keeps running. Two executors running the same task is a legal state, so the
// side effect at the end has to be guarded - here by a compare-and-set on the committed offset.
//
// The takeover also spends an attempt, so a task whose work outlives its deadline burns its budget
...
/// How long the first attempt takes. It stands in for a stalled upstream call, and it is what makes
/// the deadline pass while an executor is still running.
const STALLED_WORK_DURATION: Duration = Duration::from_secs(5);
/// How long every later attempt takes. Inside the deadline, so the takeover happens exactly once
/// and the example does not thrash between workers.
        ...
        .job(job_code.clone(), |j| {
            ...
            j.add_task(
                ...
                task_fn(move |ctx: TaskContext| {
                    let committed = Arc::clone(&committed);
                    async move { commit_offset(&committed, &ctx).await }
                }),
            );
        })
        .build()
...
async fn commit_offset(committed: &CommittedOffset, ctx: &TaskContext) -> TaskResult {
    ...
    if committed.has_reached_offset(TARGET_OFFSET) {
        tracing::info!(attempt, "offset already committed; nothing to do");
        return Ok(().into());
    }

    let work_duration = if attempt == 1 {
        STALLED_WORK_DURATION
    } else {
        NORMAL_WORK_DURATION
    };
    tracing::info!(attempt, work_seconds = work_duration.as_secs(), "starting work");
    tokio::time::sleep(work_duration).await;

    // Guard two: another executor may have committed while this one was working.
    if committed.advance_offset(TARGET_OFFSET) {
        tracing::info!(attempt, offset = TARGET_OFFSET, "committed the offset");
    } else {
        tracing::warn!(attempt, "another executor committed first; this run was a duplicate");
    }
